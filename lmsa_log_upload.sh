...
for file in /opt/apache-tomcat-8.0.22/bin/logs/inspirenetz-api.*-*-*.log; do
    zip -r "${file%.*}.zip" "$file"
done
...
for zip_file in /opt/apache-tomcat-8.0.22/bin/logs/inspirenetz-api.*-*-*.zip; do
    aws s3 cp "$zip_file" s3://inspz-app-logs/lmsa-logs/adl-prod-lmsa-01/
done
echo "completed of uploading the files to s3 "

# Command 3: Remove log files 
sudo rm /opt/apache-tomcat-8.0.22/bin/logs/inspirenetz-api.*-*-*.log
echo "original log files are removed"

# Command 4: Deleting the zip files  except last 15 days
cd /opt/apache-tomcat-8.0.22/bin/logs/
pwd
ls -1 | head -n -15 | xargs rm
echo "zipped files are removed keeping the last 20 days file"
